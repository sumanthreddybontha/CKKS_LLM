...
int main() {
    ...
    // Step 5: Perform encrypted vector addition
    auto ciphertextSum = cc->EvalAdd(ciphertext1, ciphertext2);

    // Step 6: Decrypt and decode the result
    Plaintext decryptedResult;
    cc->Decrypt(keys.secretKey, ciphertextSum, &decryptedResult);
    decryptedResult->SetLength(input1.size());

    // Step 7: Print the decrypted result
    cout << "Result of homomorphic addition: " << decryptedResult << endl;
